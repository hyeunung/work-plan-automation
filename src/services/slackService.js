...
/**
 * Slack image 블록이 렌더링 가능한 이미지 포맷인지 확인합니다.
 * (Slack은 SVG를 지원하지 않아 invalid_blocks 에러가 발생함)
 */
function isSlackRenderableImage(url) {
  if (!url) return false;
  const pathname = url.split('?')[0].toLowerCase();
  return /\.(png|jpe?g|gif|webp)$/.test(pathname);
}
...
async function sendDailyReport({ date, memberReports, targetChannelName = '일일업무보고' }) {
  try {
    ...
    for (const rep of memberReports) {
        ...
        for (const log of rep.dailyLogs) {
          ...
          if (isOutsideWork) {
            ...
            for (const img of logImages) {
              ...
              if (isSlackRenderableImage(shortUrl)) {
                memberBlocks.push({
                  type: 'image',
                  image_url: shortUrl,
                ...
                // SVG 등 Slack 미지원 포맷은 image 블록 대신 링크로 대체
                memberBlocks.push({
                  type: 'section',
                  text: {
                    type: 'mrkdwn',
                    text: `  📎 <${shortUrl}|이미지 보기: ${img.label || '이미지 첨부'}>`
                  }
                });
              }
            }
      ...
      // 한 명의 발송 실패가 전체 보고를 중단시키지 않도록 개별 격리
      try {
        await slack.chat.postMessage({
          channel: channelId,
        ...
        console.error(`  -> ❌ '${cleanName} 님' 메시지 발송 실패 (${memberErr.message}), 이미지 블록 제거 후 재시도합니다.`);
        try {
          const blocksWithoutImages = memberBlocks.filter(b => b.type !== 'image');
          await slack.chat.postMessage({
            channel: channelId,
            text: `👤 ${cleanName} 님 일일 업무 보고 - (${dayLabel})`,
          ...
          console.log(`  -> 🎉 '${cleanName} 님' 재시도(이미지 제외) 발송 완료!`);
        } catch (retryErr) {
          console.error(`  -> ❌ '${cleanName} 님' 재시도마저 실패, 다음 담당자로 계속 진행:`, retryErr.message);
        }
      }
