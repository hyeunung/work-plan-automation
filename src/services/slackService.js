 ...
 */
function buildDailySummarySection(memberReports) {
  let summary = `## 📢 오늘의 업무 요약 브리핑\n\n`;
...
async function buildDailyReportMarkdown({ date, memberReports }) {
  try {
    ...
    // [신규] 오늘의 업무 요약본 추가
    const summarySection = buildDailySummarySection(memberReports);
    if (summarySection) {
    ...
    try {
      ...
      if (slackSummary) {
        ...
        const formattedSlackSummary = slackSummary
          .replace(/## 📢 오늘의 업무 요약 브리핑/g, '📢 *[오늘의 업무 요약 브리핑]*')
